...
class TeamLogic:
    ...
    def create_team(self, name: str, captain_handle: str, player_handles: list[str]) -> Team:
        ...
        players_by_handles = {player.handle: player for player in all_players}
        for player_handle in player_handles:
            if player_handle not in players_by_handles:
                raise ValidationError(f"Leikmaður {player_handle} er ekki til")

        player = players_by_handles[player_handles]
        #checks if player belongs to a team that is not the team we are making
        #If both are True we raise error
        if player.team_name and player.team_name != name:
            raise ValidationError("Leikmaður {handle} er nú þegar skráður í liðið {player.team_name}.")
